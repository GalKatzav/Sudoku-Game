...
class Board(BoardInterface):  # Define the Board class inheriting from BoardInterface
    ...
    def reset_board(self):
        ...
        try:
            ...
            self.solution = None  # Reset the solution attribute
        except Exception as e:
    ...
    def fill_grid(self):
        ...
        try:
            ...
            while solver.count_solutions(temp_board) != 1:  # Ensure the puzzle has a unique solution
                self.shuffle_board(temp_board)
                solver.solve(temp_board)
            self.solution = [row[:] for row in temp_board]  # Store the solution
