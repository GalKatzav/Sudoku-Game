...
class SudokuMenu:
        ...
        for button in self.buttons:  # Iterate through the buttons
            if button.click(event):  # Check if a button is clicked
                if button.feedback == "New Game":
                    ...
                    self.reset_colors()  # Reset colors of all cells
    ...
    def update_cells(self):
        ...
        for row in range(9):  # Iterate through each row
            for col in range(9):  # Iterate through each column
                self.cells[row][col].set_value(self.board.grid[row][col].value, BLACK)  # Set the value for each cell
                self.cells[row][col].fixed = self.board.grid[row][col].fixed  # Set the fixed status for each cell
    ...
    def cell_click(self, pos):
        ...
        if 0 <= col < 9 and 0 <= row < 9:  # Check if the click is within the board boundaries
            ...
            if not self.selected_cell.fixed:  # Only select the cell if it's not fixed
    ...
    def key_input(self, key):
        ...
        """
        if self.selected_cell and not self.selected_cell.fixed and key in range(pygame.K_1, pygame.K_9 + 1):  # Check if a valid digit key is pressed
            ...
            self.reset_colors()  # Reset colors of all cells before checking solution
    ...
    def reset_colors(self):
        ...
        for row in range(9):  # Iterate through each row
            for col in range(9):  # Iterate through each column
                self.cells[row][col].set_color(BLACK)  # Reset to black
    ...
    def ask_replay(self):
        ...
        while not done:  # Loop until done
            ...
            prompt_surface = font.render("Congratulations! Do you want to play again? (yes/no): ", True, BLACK)  # Render prompt
